import { Ionicons } from "@expo/vector-icons";
import { Pressable, Text, View } from "react-native";

import { Card } from "@/src/components/ui/card";
import {
  formatWeightKg,
  getWeightDeltaLabel,
  getWeightDeltaTone,
} from "@/src/lib/dashboard";
import { triggerHaptic } from "@/src/lib/haptics";

type WeightEntryCardProps = {
  todayWeightKg: number | null;
  deltaKg: number | null;
  onPress: () => void;
};

export function WeightEntryCard({
  todayWeightKg,
  deltaKg,
  onPress,
}: WeightEntryCardProps) {
  const deltaTone = getWeightDeltaTone(deltaKg);
  const ctaLabel =
    todayWeightKg === null ? "Unesi jutarnju težinu" : "Izmeni jutarnju težinu";

  return (
    <Pressable
      accessibilityLabel={ctaLabel}
      accessibilityRole="button"
      className="rounded-3xl"
      onPress={() => {
        triggerHaptic("selection");
        onPress();
      }}
    >
      <Card className="flex-row items-center justify-between gap-4 px-4 py-4">
        <View className="min-w-0 flex-1 flex-row items-center gap-4">
          <View className="h-12 w-12 items-center justify-center rounded-2xl bg-[#19335E]">
            <Text className="text-lg font-black text-[#6EA8FF]">KG</Text>
          </View>

          <View className="min-w-0 flex-1 gap-1">
            <Text className="text-xl font-bold text-text" numberOfLines={2}>
              {todayWeightKg === null ? "Unesi jutarnju težinu" : "Jutarnja težina"}
            </Text>
            <Text
              className={
                deltaTone === "success"
                  ? "text-sm text-success"
                  : deltaTone === "danger"
                    ? "text-sm text-danger"
                    : "text-sm text-muted"
              }
            >
              {todayWeightKg === null
                ? "Najtačnije odmah po buđenju, posle toaleta."
                : getWeightDeltaLabel(deltaKg)}
            </Text>
          </View>
        </View>

        <View className="items-end gap-2">
          {todayWeightKg !== null ? (
            <Text
              className="text-2xl font-black text-text"
              style={{ fontVariant: ["tabular-nums"] }}
            >
              {formatWeightKg(todayWeightKg)}
            </Text>
          ) : null}
          <View className="h-12 w-12 items-center justify-center rounded-2xl bg-accent">
            <Ionicons
              color="#FFFFFF"
              name={todayWeightKg === null ? "add" : "pencil"}
              size={20}
            />
          </View>
        </View>
      </Card>
    </Pressable>
  );
}
